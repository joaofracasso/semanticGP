...
def noSameMate(individuals):
    aspirants = selRandom(individuals, 2)
    count = 0
    while(aspirants[0].fitness.values == aspirants[1].fitness.values and count<10):
        aspirants = selRandom(individuals, 2)
        count += 1
    return aspirants
...
__all__ = ['selRandom', 'selBest', 'selWorst', 'selRoulette',
           'selTournament', 'selDoubleTournament', 'noSameMate']